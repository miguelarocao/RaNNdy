...
class SentenceVAE(SentenceAutoEncoder):
    ...
    def __init__(self, data_iterator, mode=tf.estimator.ModeKeys.TRAIN, log_api_key=None):
        ...
        super().__init__(data_iterator, mode, log_api_key)
        self.params['kl_weight_scaling'] = 10 # TODO: Tune these values
        self.params['kl_weight_offset'] = 5000
    ...
    def _build_encoder(self):
        ...
        self.encoding_mean = tf.layers.dense(encoder_state, self.params['lstm_size'], activation=None,
                                             name="linear_mean_layer")
        self.encoding_log_var = tf.layers.dense(encoder_state, self.params['lstm_size'], activation=None,
                                                name="linear_var_layer")
    ...
    def _build_trainer(self):
        ...
        # Build KL loss
        kl_loss = self.kl_divergence_multigauss(self.encoding_mean, tf.exp(self.encoding_log_var))
        kl_loss /= self.params['batch_size']
        ...
        self.kl_weight = tf.sigmoid(tf.assign_add(self.train_step_count, 1) * self.params['kl_weight_scaling']
                                    + self.params['kl_weight_offset'])

        # Total loss
        total_loss = reconstruction_loss + self.kl_weight*kl_loss

        # Populate metric dict
        self.loss_metrics[MetricType.TOTAL_LOSS] = total_loss
        self.loss_metrics[MetricType.RECONSTRUCTION_LOSS] = reconstruction_loss
        self.loss_metrics[MetricType.KL_LOSS] = kl_loss
